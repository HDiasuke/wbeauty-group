...
function the_dp_footer_logo(){
  ...
  if($logo){
    ...
    echo '<a href="' . $url . '/" title="' . $title . '"><img class="f_logo" src="' . $logo['url'] . '" alt="'. $title . '" title="' . $title .'" /></a>' . "\n";
    echo "</h3>\n";
    if ($option['footer_description']) {
      echo "<div id='footer_description2'>\n";
      echo wpautop(stripslashes($option['footer_description']));
      echo "</div>\n";
    };
    echo "</div>\n";
  } else {
    ...
    // $optionはarray
    // $optionのkeyの存在チェック
    if (array_key_exists('footer_description', $option)) {
    	// $optionのvalueの存在チェック
    	if ($option['footer_description']) {
      		echo "<div id='footer_description2'>\n";
      		echo wpautop(stripslashes($option['footer_description']));
