  ...
  <div class="inline-edit-col column-meta-seo">
    <div class="inline-edit-group">
      ...
      <label class="inline-edit-tcd-w_meta_description">
        <span class="title" style="line-height:1.5;"><?php _e('Meta description', 'tcd-w'); ?></span>
...
<?php
    ...
    if ( $post_type == 'post' ) {
      ...
      echo '<input type="hidden" name="recommend_post2_meta_box_nonce" value="', wp_create_nonce( 'recommend2.php' ), '" />';
      echo '<input type="hidden" name="recommend_post3_meta_box_nonce" value="', wp_create_nonce( 'recommend3.php' ), '" />';
      echo '<input type="hidden" name="pickup_post_meta_box_nonce" value="', wp_create_nonce( 'pickup.php' ), '" />';
...
<fieldset class="inline-edit-col-right">
  <div class="inline-edit-col column-recommend">
    <div class="inline-edit-group">
      ...
      <label style="line-height:1.5;"><input type="checkbox" name="recommend_post3" value="on" /> <?php _e( 'Show this post for recommend post3.', 'tcd-w' );  ?></label>
      <label style="line-height:1.5;"><input type="checkbox" name="pickup_post" value="on" /> <?php _e( 'Show this post for pickup post.', 'tcd-w' );  ?></label>
    </div>
  </div>
</fieldset>
<?php
...
function custom_quick_edit_values( $actions, $post ) {
	...
	// seo
    if ( in_array( $post->post_type, array( 'post', 'page', 'news' ) ) ) {
		$meta_keys = array_merge( $meta_keys, array( 'tcd-w_meta_title', 'tcd-w_meta_description' ) );
	}

    // recommend
    if ( $post->post_type == 'post' ) {
		$meta_keys = array_merge( $meta_keys,  array( 'recommend_post', 'recommend_post2', 'recommend_post3', 'pickup_post') );
	}
